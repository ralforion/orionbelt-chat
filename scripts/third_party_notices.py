...
PROJECT_NAME = "OrionBelt Chat"
ROOT_PACKAGE = "orionbelt-chat"
MODULE_NAME = "orionbelt_chat"
...
# Extras this file accounts for. The image installs no extra, and the only one
# defined is `dev` — tooling the project runs rather than a work it ships — so the
# noticed closure and the redistributed closure are the same set.
NOTICED_EXTRAS: tuple[str, ...] = ()

# Nothing to resolve: the Dockerfile's `uv sync` takes no `--extra`.
